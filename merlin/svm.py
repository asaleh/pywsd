    ...
    def train(self, x, y):
        ...
        for epoch_nr in xrange(self.nr_epochs):
            ...
            for nr in xrange(nr_x):
                ...
                objective += 0.5 * self.regularizer * l2norm_squared(w) - scores[:,
                                                                          y_true] + cost_augmented_loss[
                                                                                    :, y_hat]
            ...
            self.trained = False
            print
            "Epochs: %i Objective: %f" % (epoch_nr, objective)
            print
            "Epochs: %i Accuracy: %f" % (epoch_nr, acc)
        self.trained = True
        return w
