...
class SimpleDataSet:
    ...
    def __init__(self, nr_examples=100, g1=[[-5, -5], 1], g2=[[5, 5], 1], balance=0.5,
                 split=[0.8, 0, 0.2]):
        ...
        self.X = X[perm, :]
        self.y = y[perm]
        train_y, dev_y, test_y, train_X, dev_X, test_X = split_train_dev_test(self.X, self.y,
                                                                              split[0], split[1],
                                                                              split[2])
    ...
    def get_name(self):
        ...
        self.nr_points, self.balance, self.split[0], self.split[1], self.split[2])
...
def split_train_dev_test(X, y, train_per, dev_per, test_per):
    if (train_per + dev_per + test_per > 1):
        print
        "Train Dev Test split should sum to one"
        return
    dim = y.shape[0]
